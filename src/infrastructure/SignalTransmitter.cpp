...
void SignalTransmitter::tick(){if(!running())return;const auto now=m_clock.nsecsElapsed()/1000;QString error;
    ...
    }else{int budget=256;while(m_once<m_plan.items.size()&&budget--){const auto&i=m_plan.items[m_once++];if(!i.enabled)continue;if(send(i,error))++m_status.sent[i.key];else m_status.failures[i.key]=error;}
...
void SignalTransmitter::update(const PayloadUpdate&u){if(!running()||u.run!=m_plan.run||u.connection!=m_plan.connection)return;
    QString error;if(u.enableChanged){if(m_lin){if(!m_lin->setEnabled(u.key,u.enabled,m_clock.nsecsElapsed()/1000,error))fail(error);}else m_scheduler.setEnabled(u.key,u.enabled);return;}if(m_lin){if(!m_lin->update(u.key,u.payload,error))fail(error);}else{m_scheduler.update(u.key,u.payload);for(auto&i:m_plan.items)if(i.key==u.key&&u.payload.revision>i.revision&&u.payload.bytes.size()==i.payload.size()){i.payload=u.payload.bytes;i.revision=u.payload.revision;}}}
