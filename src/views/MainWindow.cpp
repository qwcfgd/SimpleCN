...
void MainWindow::createChannelDialog(ChannelViewModel*channel) {
    ...
    auto*files=new QHBoxLayout;auto*load=new QPushButton("载入配置");load->setObjectName("loadSettings");auto*save=new QPushButton("保存通道配置");save->setObjectName("saveSettings");files->addWidget(load);files->addWidget(save);form->addRow(files);form->addRow(error);form->addRow(buttons);
    ...
    connect(save,&QPushButton::clicked,&dialog,[&]{if(target&&!apply())return;QString message;const bool ok=m_configuration.save(m_channels,message);showError(ok?QString("已保存 %1 个软件通道").arg(m_channels.size()):message);});
