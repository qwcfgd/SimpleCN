...
SignalPlotDialog::SignalPlotDialog(SignalTransmitViewModel*vm,QWidget*parent):QDialog(parent),m_vm(vm){
    ...
    m_canvas=new SignalPlotCanvas(m_model);auto *scroll=new QScrollArea;scroll->setWidgetResizable(true);scroll->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);scroll->setWidget(m_canvas);scroll->setFrameShape(QFrame::NoFrame);split->addWidget(scroll);split->setSizes({375,875});split->setStretchFactor(0,3);split->setStretchFactor(1,7);root->addWidget(split,1);
