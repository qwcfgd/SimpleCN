...
SignalTransmitViewModel::SignalTransmitViewModel(Bus bus,QObject*parent):QObject(parent),m_bus(bus){
    auto empty=QSharedPointer<DatabaseDefinition>::create();empty->bus=bus;m_database=empty;if(bus==Bus::Lin)m_work.node="Master";
    connect(&m_replayLoad,&QFutureWatcher<TraceLog>::finished,this,[this]{m_importing=false;auto result=m_replayLoad.result();if(result.error.isEmpty()){remember();m_replayLog=std::move(result);m_work.replaySettings["path"]=m_replayPath;m_message=QString("已导入 %1 条回放记录，%2 个日志通道；跳过 %3 条非正常或不支持的记录").arg(m_replayLog.frames.size()).arg(m_replayLog.channels.size()).arg(m_replayLog.skipped);}else m_message=result.error;emit changed();emit notice(m_message);});
...
bool SignalTransmitViewModel::install(Database db,QString&error){
    ...
    WorkingSet work;work.repeatCount=m_work.repeatCount;work.uiSettings=m_work.uiSettings;work.replaySettings=m_work.replaySettings;work.node=db->master;work.schedules=db->schedules;if(!work.schedules.isEmpty())work.schedule=work.schedules.first().name;
...
bool SignalTransmitViewModel::commit(const QString&key,QString&error){
    ...
    if(running())emit payloadRequested({m_replaying?frameKey(m_bus,f->id,f->extended):key,draft.applied,m_status.run,m_connection});return true;
}
...
bool SignalTransmitViewModel::editPayload(const QString&key,const QString&input,QString&error){
    ...
    if(running())emit payloadRequested({m_replaying?frameKey(m_bus,f->id,f->extended):key,draft.applied,m_status.run,m_connection});changedFrame(key);return true;
}
bool SignalTransmitViewModel::setFrameEnabled(const QString&key,bool enabled,QString&error){
    ...
    if(running())emit payloadRequested({m_replaying?frameKey(m_bus,f->id,f->extended):key,m_work.frames[key].applied,m_status.run,m_connection,enabled,true});changedFrame(key);return true;
}
...
bool SignalTransmitViewModel::putCustom(const QString&oldKey,const QString&name,quint32 id,int length,int cycle,QString&error){
    if(!canStructure()||name.trimmed().isEmpty()||id>(m_bus==Bus::Can?0x1fffffff:61)||length<(m_bus==Bus::Can?0:1)||length>8||cycle<0||(m_bus==Bus::Lin&&id>=60&&length!=8)){error="自建帧需要有效名称、0–0x1FFFFFFF ID、0–8 字节及非负整数周期";return false;}
    ...
    remember();FrameDefinition f;f.key=frameKey(m_bus,id,id>0x7ff);f.id=id;f.extended=id>0x7ff;f.length=length;f.name=name.trimmed();f.custom=true;f.cycleMs=cycle;f.classicChecksum=m_bus==Bus::Lin&&id>=60;f.publisher=m_work.node;
    ...
    draft.enabled=true;m_work.customFrames.append(f);m_work.frames[f.key]=draft;
    if(m_bus==Bus::Lin)for(auto &s:m_work.schedules)if(s.name==m_work.schedule)s.entries.append({f.key,QString::number(cycle>0?cycle:100),{},0});emit structureChanged();emit changed();return true;
}
...
bool SignalTransmitViewModel::setRole(LinRole role,const QString&node,QString&error){
    ...
    if(m_database->path.isEmpty())selected=role==LinRole::Master?"Master":role==LinRole::Slave?"Slave":QString();
    if(!m_database->path.isEmpty()&&((role==LinRole::Master&&selected!=m_database->master)||(role==LinRole::Slave&&(!m_database->nodes.contains(selected)||selected==m_database->master)))){error="角色与实际节点不匹配";return false;}
...
bool SignalTransmitViewModel::selectSchedule(const QString&name,QString&error){
    if(replayRunning()||m_status.cleanupPending){error="回放或硬件停止未确认期间不能切换调度表";return false;}
...
void SignalTransmitViewModel::restoreSnapshot(const WorkingSet&snapshot){
    ...
    const bool restoreUi=next.uiSettings!=m_work.uiSettings;const bool reload=next.replaySettings.value("path")!=m_work.replaySettings.value("path");m_work=next;if(restoreUi)emit configurationRestored();emit structureChanged();emit changed();if(reload){m_replayLog={};if(replayConfigured())importReplay(m_work.replaySettings.value("path").toString());}
}
...
bool SignalTransmitViewModel::start(bool periodic,QString&error){
    if(replayConfigured()){if(!canStructure()){error="请先停止当前任务";return false;}emit replayRequested(periodic);return true;}
    if(!hasEnabled()){error="请使能至少一条待发送报文或帧";return false;}
    if(m_bus==Bus::Lin&&!periodic&&m_work.role!=LinRole::Master){error="从节点和观测节点不支持多次调度";return false;}
    ...
    plan.periodic=periodic;plan.repeatCount=m_work.repeatCount;plan.connection=m_connection;plan.databaseRevision=m_database->sha256;
    if(m_bus==Bus::Lin){
        ...
        if(!m_database->path.isEmpty()&&((plan.role==LinRole::Master&&plan.node!=m_database->master)||(plan.role==LinRole::Slave&&(!m_database->nodes.contains(plan.node)||plan.node==m_database->master)))){error="请选择有效的实际节点";return false;}
    }
    for(const auto&f:definitions()){const auto &draft=m_work.frames[f.key];if(m_bus==Bus::Can&&!draft.enabled)continue;
        ...
        plan.items.append({f.key,f.id,f.extended,draft.applied.bytes,draft.cycleMs,draft.applied.revision,m_bus==Bus::Lin&&(f.custom||(f.id==61&&m_work.role==LinRole::Slave))?m_work.node:f.publisher,f.classicChecksum,draft.sendEnabled});
    }
    if(m_bus==Bus::Can&&plan.items.isEmpty()){error="请先选择节点报文或右键自建报文";return false;}
    m_replaying=false;m_periodic=periodic;plan.run=++m_nextRun;m_status={};m_status.state=RunState::Starting;m_status.run=plan.run;m_status.detail="启动中";emit startRequested(plan);emit changed();return true;
}
void SignalTransmitViewModel::stop(){emit replayStopRequested();if(!signal::active(m_status.state)&&!m_status.cleanupPending)return;m_status.state=RunState::Stopping;emit stopRequested(m_status.run);emit changed();}
...
void SignalTransmitViewModel::observeFrames(FrameBatch frames){
    for(auto &r:frames){if(m_traceOrigin<0)m_traceOrigin=r.captureUs;r.timeUs=r.captureUs-m_traceOrigin;}
    m_traceHistory+=frames;if(m_traceHistory.size()>10000)m_traceHistory.remove(0,m_traceHistory.size()-10000);
    emit framesObserved(frames);
}
void SignalTransmitViewModel::clearTrace(){m_traceHistory.clear();m_traceOrigin=-1;emit traceCleared();}
...
void SignalTransmitViewModel::applyConfiguration(Database database,const WorkingSet&work){
    m_replayLog={};m_database=database;m_work=work;m_baseline=work;m_hasBaseline=true;m_history.clear();m_status={};
    emit configurationRestored();emit structureChanged();emit changed();
    if(replayConfigured())importReplay(m_work.replaySettings.value("path").toString());
}
}

namespace host {
bool SignalTransmitViewModel::hasEnabled()const{
    if(m_bus==signal::Bus::Can){for(const auto &f:queuedDefinitions())if(m_work.frames.value(f.key).sendEnabled)return true;}
    else for(const auto &s:m_work.schedules)if(s.name==m_work.schedule)for(const auto &slot:s.entries)if(m_work.frames.value(slot.frame).sendEnabled)return true;
    return false;
}
void SignalTransmitViewModel::setRepeatCount(int count){if(!canStructure())return;count=qBound(1,count,1000000);if(m_work.repeatCount==count)return;remember();m_work.repeatCount=count;emit changed();}
void SignalTransmitViewModel::setUiSettings(const QJsonObject &value){if(m_work.uiSettings==value)return;m_work.uiSettings=value;emit changed();}
bool SignalTransmitViewModel::createSchedule(QString &error){
    if(!canStructure()||m_bus!=signal::Bus::Lin){error="停止后才能新建调度表";return false;}
    QSet<QString> names;for(const auto &s:m_work.schedules)names.insert(s.name);
    int n=1;QString name;do{name=QString("Schedule_Default%1").arg(n++,3,10,QChar('0'));}while(names.contains(name));
    remember();signal::Schedule s;s.name=name;m_work.schedules.append(s);m_work.schedule=name;emit structureChanged();emit changed();return true;
}
bool SignalTransmitViewModel::setLinDelay(int row,const QString &value,QString &error){
    bool ok=false;const double delay=value.toDouble(&ok);
    if(!canStructure()||!ok||!std::isfinite(delay)||delay<=0||delay>65535){error="delay 必须在 0–65535 ms 之间";return false;}
    for(int n=0;n<m_work.schedules.size();++n)if(m_work.schedules.at(n).name==m_work.schedule&&row>=0&&row<m_work.schedules.at(n).entries.size()){remember();m_work.schedules[n].entries[row].delayMs=value;emit changed();return true;}
    error="调度槽不存在";return false;
}
bool SignalTransmitViewModel::changeId(const QString &oldKey,quint32 id,QString &error){
    using namespace signal;
    const auto *old=frame(oldKey);if(!canStructure()||!old||id>(m_bus==Bus::Can?0x1fffffff:61)){error="停止后才能修改有效 ID";return false;}
    if(id==old->id)return true;
    if(m_bus==Bus::Can){for(const auto &f:queuedDefinitions())if(f.id==id){error="待发送表已有目标 ID，已撤回变更";return false;}}
    else for(const auto &s:m_work.schedules)if(s.name==m_work.schedule)for(const auto &slot:s.entries){const auto *f=frame(slot.frame);if(f&&f->id==id){error="本调度表已有目标 ID，已撤回变更";return false;}}
    FrameDefinition replacement=*old;bool hit=false;
    for(const auto &f:m_database->frames)if(f.id==id){replacement=f;hit=true;break;}
    const QString databaseKey=frameKey(m_bus,id,hit?replacement.extended:id>0x7ff);
    const QString target=m_bus==Bus::Lin?databaseKey+"@"+QString::fromLatin1(m_work.schedule.toUtf8().toHex()):databaseKey;
    if(!hit){replacement.key=target;replacement.id=id;replacement.extended=id>0x7ff;replacement.custom=true;replacement.fields.clear();replacement.issue.clear();replacement.name=QString("Frame_%1").arg(id,0,16).toUpper();replacement.cycleMs=100;replacement.publisher=m_work.node;replacement.transmitters.clear();replacement.classicChecksum=m_bus==Bus::Lin&&id>=60;if(replacement.classicChecksum)replacement.length=8;}
    replacement.key=target;
    TxDraft draft;if(!SignalCodec::initialize(replacement,m_bus,draft,error))return false;
    draft.enabled=true;draft.sendEnabled=m_work.frames.value(oldKey).sendEnabled;
    if(!hit){draft.applied.bytes=m_work.frames.value(oldKey).applied.bytes.leftJustified(replacement.length,char(0),true);draft.appliedValues=draft.values;}
    QString delay=QString::number(draft.cycleMs);if(hit&&m_bus==Bus::Lin){bool found=false;for(const auto &s:m_database->schedules){for(const auto &slot:s.entries)if(slot.frame==databaseKey){delay=slot.delayMs;found=true;break;}if(found)break;}}
    remember();
    if(m_bus==Bus::Lin||!hit){bool replaced=false;for(auto &f:m_work.customFrames)if(f.key==target){f=replacement;replaced=true;break;}if(!replaced)m_work.customFrames.append(replacement);}
    m_work.frames[target]=draft;
    if(m_bus==Bus::Can)m_work.frames[oldKey].enabled=false;
    else for(auto &s:m_work.schedules)if(s.name==m_work.schedule)for(auto &slot:s.entries)if(slot.frame==oldKey){slot.frame=target;slot.delayMs=delay;slot.issue.clear();}
    emit structureChanged();emit changed();return true;
}
}

namespace host {
void SignalTransmitViewModel::importReplay(const QString &path){
    if(!canStructure()||path.isEmpty())return;m_importing=true;m_replayPath=path;m_message="正在读取回放日志…";emit changed();
    m_replayLoad.setFuture(QtConcurrent::run([path]{return TraceReader::read(path);}));
}
void SignalTransmitViewModel::resetReplay(){if(!canStructure())return;remember();m_work.replaySettings={};m_replayLog={};m_message.clear();emit changed();}
void SignalTransmitViewModel::setReplayMapping(const QJsonObject &mapping){if(!canStructure())return;remember();m_work.replaySettings["mapping"]=mapping;emit changed();}
bool SignalTransmitViewModel::startReplayPlan(signal::TxPlan plan,QString &error){
    if(!canStart()){error="回放目标通道未连接或正在执行其他任务";return false;}
    m_replaying=true;plan.run=++m_nextRun;plan.connection=m_connection;m_status={};m_status.run=plan.run;m_status.state=signal::RunState::Starting;m_periodic=plan.periodic;emit startRequested(plan);emit changed();return true;
}
void SignalTransmitViewModel::setReplayGroupRunning(bool on){m_replayGroupRunning=on;emit changed();}
}
