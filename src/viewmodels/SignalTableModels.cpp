...
QVariant SignalValueTableModel::data(const QModelIndex&i,int role)const{
    ...
    case 5:return m_defaults?(s.initial.isEmpty()?QString("程序默认（文件未定义）"):s.initialSource+" = "+s.initial):s.comment;
...
Qt::ItemFlags SignalValueTableModel::flags(const QModelIndex&i)const{auto flags=QAbstractTableModel::flags(i);const auto*f=m_vm->frame(m_key);if(!f||!i.isValid())return flags;
    if((m_defaults?m_vm->canStructure():m_vm->canData())&&f->issue.isEmpty()&&(i.column()==2||(i.column()==3&&(f->fields[i.row()].conversion||!f->fields[i.row()].labels.isEmpty())&&!f->fields[i.row()].array)))flags|=Qt::ItemIsEditable;return flags;
}
...
QVariant CanTxTableModel::headerData(int c,Qt::Orientation o,int role)const{if(role!=Qt::DisplayRole)return {};if(o==Qt::Vertical)return c+1;return QStringList{"序号","名称","ID","类型","字节","周期 ms","报文"}.value(c);}
QVariant CanTxTableModel::data(const QModelIndex&i,int role)const{
    ...
    if(role!=Qt::DisplayRole&&role!=Qt::EditRole)return {};
    switch(i.column()){
    case 0:return i.row()+1;case 1:return f.name;case 2:return "0x"+QString::number(f.id,16).toUpper();case 3:return QString(f.extended?"29 bit":"11 bit")+(f.custom?" · 自建":" · DBC");case 4:return f.length;case 5:return d.cycleMs;
    case 6:return role==Qt::EditRole&&!d.frameError.isEmpty()?d.frameInput:QString::fromLatin1(d.applied.bytes.toHex(' ')).toUpper();
    }return {};
}
Qt::ItemFlags CanTxTableModel::flags(const QModelIndex&i)const{
    ...
    if((i.column()==5&&m_vm->canStructure())||(i.column()==6&&m_vm->canData()&&m_definitions[i.row()].issue.isEmpty()))flags|=Qt::ItemIsEditable;return flags;
}
bool CanTxTableModel::setData(const QModelIndex&i,const QVariant&v,int role){
    ...
    if(i.column()==6)ok=m_vm->editPayload(key(i.row()),v.toString(),error);
    else if(i.column()==5){bool parsed=false;int period=v.toString().toInt(&parsed);if(parsed&&period>=0)ok=m_vm->setCanOptions(key(i.row()),true,period,error);else error="周期须为非负整数 ms；0 表示仅单次";}
...
QVariant LinScheduleTableModel::headerData(int c,Qt::Orientation o,int role)const{if(role!=Qt::DisplayRole)return {};if(o==Qt::Vertical)return c+1;return QStringList{"槽","帧","发布者","delay ms","报文","帧头 · 数据发送类型"}.value(c);}
