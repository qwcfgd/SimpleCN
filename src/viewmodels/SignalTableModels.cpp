...
CanTxTableModel::CanTxTableModel(SignalTransmitViewModel*vm,QObject*p):QAbstractTableModel(p),m_vm(vm),m_definitions(vm->definitions()){
    connect(vm,&SignalTransmitViewModel::structureChanged,this,[this]{beginResetModel();m_definitions=m_vm->definitions();endResetModel();});
...
QVariant CanTxTableModel::headerData(int c,Qt::Orientation o,int role)const{if(role!=Qt::DisplayRole)return {};if(o==Qt::Vertical)return c+1;return QStringList{"启用","报文","ID","类型","字节","周期 ms","状态","已提交","漏期"}.value(c);}
QVariant CanTxTableModel::data(const QModelIndex&i,int role)const{if(!i.isValid()||i.row()>=m_definitions.size())return {};const auto&f=m_definitions[i.row()];const auto&d=m_vm->working().frames[f.key];const auto&status=m_vm->status();
    if(role==Qt::CheckStateRole&&i.column()==0)return d.enabled?Qt::Checked:Qt::Unchecked;
    if(role==Qt::ToolTipRole)return f.comment+"\n"+f.issue;
    if(role==Qt::BackgroundRole&&(!f.issue.isEmpty()||status.failures.contains(f.key)))return QBrush(QColor("#ffe0e0"));
    if(role!=Qt::DisplayRole&&role!=Qt::EditRole)return {};
    switch(i.column()){case 1:return f.name;case 2:return "0x"+QString::number(f.id,16).toUpper();case 3:return QString(f.extended?"29 bit":"11 bit")+(f.custom?" · 自建":" · DBC");case 4:return f.length;case 5:return d.cycleMs;
    case 6:return !f.issue.isEmpty()?f.issue:status.failures.contains(f.key)?status.failures.value(f.key):!d.errors.isEmpty()||!d.frameError.isEmpty()?"错误草稿 / 继续有效值":d.enabled?"已启用":"未启用";
    case 7:return QString::number(status.sent.value(f.key));case 8:return QString::number(status.missed.value(f.key));}return {};
}
Qt::ItemFlags CanTxTableModel::flags(const QModelIndex&i)const{auto flags=QAbstractTableModel::flags(i);if(m_vm->canStructure()&&i.isValid()){if(i.column()==0&&m_definitions[i.row()].issue.isEmpty())flags|=Qt::ItemIsUserCheckable;if(i.column()==5)flags|=Qt::ItemIsEditable;}return flags;}
bool CanTxTableModel::setData(const QModelIndex&i,const QVariant&v,int role){if(!m_vm->canStructure()||!i.isValid())return false;const auto&d=m_vm->working().frames[key(i.row())];QString error;bool ok=false;
    if(i.column()==0&&role==Qt::CheckStateRole)ok=m_vm->setCanOptions(key(i.row()),v.toInt()==Qt::Checked,d.cycleMs,error);
    else if(i.column()==5&&role==Qt::EditRole){bool parsed=false;int period=v.toString().toInt(&parsed);if(parsed&&period>=0)ok=m_vm->setCanOptions(key(i.row()),d.enabled,period,error);else error="周期须为非负整数 ms；0 表示仅单次";}
...
void SignalTreeModel::rebuild(){clear();setHorizontalHeaderLabels({"节点 / 报文","ID / 关系"});QMap<QString,QStandardItem*> nodes;
    ...
    for(const auto&f:m_vm->definitions()){append(all,f,f.custom?"自建":"数据库");if(nodes.contains(f.publisher))append(nodes[f.publisher],f,"发布");QSet<QString> receivers;for(const auto&s:f.fields)for(const auto&r:s.receivers)receivers.insert(r);for(const auto&r:receivers)if(nodes.contains(r)&&r!=f.publisher)append(nodes[r],f,"订阅");}
}
LinScheduleTableModel::LinScheduleTableModel(SignalTransmitViewModel*vm,QObject*p):QAbstractTableModel(p),m_vm(vm){auto update=[this]{const auto next=m_vm->working().schedule;if(next!=m_schedule){beginResetModel();m_schedule=next;endResetModel();}else if(rowCount())emit dataChanged(index(0,0),index(rowCount()-1,4));};connect(vm,&SignalTransmitViewModel::changed,this,update);connect(vm,&SignalTransmitViewModel::structureChanged,this,[this]{beginResetModel();m_schedule=m_vm->working().schedule;endResetModel();});}
...
QVariant LinScheduleTableModel::headerData(int c,Qt::Orientation o,int role)const{if(role!=Qt::DisplayRole)return {};if(o==Qt::Vertical)return c+1;return QStringList{"槽","帧","发布者","delay ms","执行能力"}.value(c);}
QVariant LinScheduleTableModel::data(const QModelIndex&i,int role)const{const auto*s=schedule();if(!i.isValid()||!s||i.row()>=s->entries.size()||role!=Qt::DisplayRole)return {};const auto&slot=s->entries[i.row()];const auto*f=m_vm->frame(slot.frame);
    switch(i.column()){case 0:return i.row()+1;case 1:return f?f->name:slot.frame;case 2:return f?f->publisher:QString();case 3:return slot.delayMs;case 4:return !slot.issue.isEmpty()?slot.issue:f&&!f->issue.isEmpty()?f->issue:m_vm->working().role==LinRole::Monitor?"仅监听，不发布或响应":f&&f->publisher==m_vm->working().node?"当前节点发布":"外部节点响应";}return {};
}
}
