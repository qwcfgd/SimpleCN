...
LinScheduleTableModel::LinScheduleTableModel(SignalTransmitViewModel*vm,QObject*p):QAbstractTableModel(p),m_vm(vm),m_schedule(vm->working().schedule){auto update=[this]{const auto next=m_vm->working().schedule;if(next!=m_schedule){beginResetModel();m_schedule=next;endResetModel();}else if(rowCount())emit dataChanged(index(0,0),index(rowCount()-1,9));};connect(vm,&SignalTransmitViewModel::changed,this,update);connect(vm,&SignalTransmitViewModel::structureChanged,this,[this]{beginResetModel();m_schedule=m_vm->working().schedule;endResetModel();});}
...
QVariant LinScheduleTableModel::headerData(int c,Qt::Orientation o,int role)const{if(role!=Qt::DisplayRole)return {};if(o==Qt::Vertical)return c+1;return QStringList{"使能","槽","帧","ID","发布节点","类型","字节","delay ms","报文","帧头 · 数据发送类型"}.value(c);}
QVariant LinScheduleTableModel::data(const QModelIndex&i,int role)const{const auto*s=schedule();if(!i.isValid()||!s||i.row()>=s->entries.size()||(role!=Qt::DisplayRole&&role!=Qt::EditRole&&role!=Qt::ToolTipRole&&role!=Qt::BackgroundRole&&role!=Qt::CheckStateRole))return {};const auto&slot=s->entries[i.row()];const auto*f=m_vm->frame(slot.frame);
    ...
    switch(i.column()){case 1:return i.row()+1;case 2:return f?f->name:slot.frame;case 3:return f?QString("0x%1").arg(f->id,2,16,QChar('0')).toUpper():QString();case 4:return f?f->publisher:QString();case 5:if(slot.issue.contains("Event_triggered_frames"))return "事件触发帧";if(slot.issue.contains("Sporadic_frames"))return "偶发帧";return f?(f->id>=60?QString("诊断帧"):QString("无条件帧")):QString("配置命令槽");case 6:return f?QVariant(f->length):QVariant();case 7:return slot.delayMs;case 8:return role==Qt::EditRole&&!d.frameError.isEmpty()?d.frameInput:QString::fromLatin1(d.applied.bytes.toHex(' ')).toUpper();case 9:{const auto role=m_vm->working().role;const bool tx=role!=LinRole::Monitor&&f&&(f->publisher==m_vm->working().node||(f->id==61&&role==LinRole::Slave));return QString(role==LinRole::Master?"Tx":"Rx")+" · "+(tx?"Tx":"Rx");}}return {};
}
Qt::ItemFlags LinScheduleTableModel::flags(const QModelIndex&i)const{
    ...
    if(i.isValid()&&i.column()==8&&f&&f->issue.isEmpty()&&m_vm->canData())result|=Qt::ItemIsEditable;return result;
}
