...
- 报文监视时刻（rt）以 ms 显示，省略小数末尾无效的 0：`1000000 μs → 1000 ms`、`1001250 μs → 1001.25 ms`；保留有效的小数精度。
...
图像 arrange 布局在当前窗口高度内显示所有曲线，枚举 Y 轴按缩放生成刻度与参考格，命中枚举的刻度及左侧 y 列显示枚举名称。报文 rt 最多保留六位有效小数，以毫秒显示并省略小数末尾的 0（例如 `1200123.674 μs → 1200.123674 ms`）。
...
**验证范围：** V1.4.2 已完成 Qt 5.15.19 / Qt 6.8.4 Release 构建，两套各 16 项回归最终通过；原生打开、保存、中文空格路径和取消均完成桌面核查。已使用同星 CAN1（500 kbit/s）分别接收真实报文，验证 rt 的毫秒单位及去尾零显示。此前版本的 CAN1 / CAN2 外部回环记录仍保留；本次未验证在线发送中修改负载、LIN ECU 收发或实机下载。详见 [1.4.2 发布说明](docs/Release-1.4.2.md) 和 [同星硬件适配](docs/Tosun-Hardware.md)。
