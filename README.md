...
- 新增默认关闭的“暂停更新”：画面暂停、后台记录继续；左侧历史滚动条可浏览全部缓存，关闭暂停后回到最新数据。新增控件、数量及提示支持简中 / Eng。详见 [报文监视](docs/Trace-and-Graphics.md)。
- 本次 Qt 5 / Qt 6 各 16 项回归最终通过，包含超过 30,000 条报文的完整导出、暂停浏览及中英文界面核查；按用户要求不进行实机测试。

## V1.4.2：Windows 原生文件对话框

- 镜像、DBC / LDF、CDD、回放与通道配置载入，以及报文 / 运行日志导出，统一使用 Windows 现代原生打开 / 保存对话框。
- 删除 V1.4.1 的模拟顶部路径栏，使用系统自带地址栏和文件名输入框；保留初始目录、扩展名筛选、取消行为及实际文件处理逻辑。
- 应用标题和任务界面继续支持简中 / Eng；原生对话框的系统按钮、导航区和提示语言由 Windows 控制。
- 当前报文时刻（rt）按最新需求以 s 显示：`1000000 μs → 1 s`、`1001250 μs → 1.00125 s`。

## V1.4.1 修复

- 发送期间可持续编辑 raw、物理值及枚举；后台收发、计数和语言刷新不会覆盖输入，提交后更新后续发送负载。
- 枚举图像保留物理坐标：匹配位置显示枚举名称，其余刻度显示整数，参考格同步。
- 三个任务页面统一底色。该版本添加的文件选择器路径栏已在 V1.4.2 移除，由 Windows 原生控件提供路径输入。
- rt 明确以毫秒显示：实际经过 1 秒增加约 1000 ms。V1.4.1 的实机验收当时因设备移除未完成；V1.4.2 进一步去除显示末尾无效的 0。
...
图像 arrange 布局在当前窗口高度内显示所有曲线，枚举 Y 轴按缩放生成刻度与参考格，命中枚举的刻度及左侧 y 列显示枚举名称。报文 rt 以秒显示并省略小数末尾的 0（例如 `1200123.674 μs → 1.200123674 s`）。
...
| **报文监视** | t / rt / dt 列显隐、筛选、跟随、按 ID 原位更新；半字节变化着色；数据库信号展开；完整会话缓存、暂停和历史浏览；每个显示窗口最多 10,000 条。 |
...
`t` 为系统时间，`rt` 为从记录开始起算的秒时刻，`dt` 为相邻记录时间差（秒）。默认仅显示 `rt`；“滚动显示”开启后按相同 ID 原位更新。
...
**验证范围：** V1.4.2 已完成 Qt 5.15.19 / Qt 6.8.4 Release 构建，两套各 16 项回归最终通过；原生打开、保存、中文空格路径和取消均完成桌面核查。已使用同星 CAN1（500 kbit/s）分别接收真实报文，验证当时的 rt 毫秒单位及去尾零显示；当前后续修订按最新需求改为秒。此前版本的 CAN1 / CAN2 外部回环记录仍保留；本次未验证在线发送中修改负载、LIN ECU 收发或实机下载。详见 [1.4.2 发布说明](docs/Release-1.4.2.md) 和 [同星硬件适配](docs/Tosun-Hardware.md)。
